...
1. `src/products/` (module/controller/service + `dto/create-product.dto.ts`, `create-variant.dto.ts`) — CRUD productos y variantes ([§6.1](technical_guide.md#61-productcreateresponse-éxito)/[§6.2](technical_guide.md#62-variantcreateresponse-éxito); componentes de variante: [PDR §2.2](../business/pdr.md#22-variantes-y-componentes)).
2. `src/shifts/` **mínimo** — `GET /shift-periods` (catálogo; seed ya crea Mañana/Noche), `POST /shifts/open` ([§6.10](technical_guide.md#610-cashopenresponse-éxito); el período se **declara**, jamás se infiere del reloj — [PDR §13.3](../business/pdr.md#133-decisiones-residuales-pendientes-de-cierre-antes-de-v1)) y el contador `lastOrderNumber`.
...
6. `src/customers/` **(mínimo)** — `GET /customers?search=` (búsqueda por CI / NIT / nombre). **Solo lectura** en Sprint 1: habilita el componente G del POS (selector de cliente en el resumen de la orden) para que la cajera pueda buscar y asociar clientes pre-cargados vía seed. Los endpoints de escritura (`POST`, `PATCH`, `PATCH /{id}/toggle-active`) y `GET /customers/{id}/orders` llegan en Sprint 4 (paso 1).
   - `@Roles('CASHIER', 'ADMIN')` para este endpoint; `SUPER_ADMIN` pasa siempre por la regla global del `RolesGuard` ([§5.2](technical_guide.md#52-autenticación-y-autorización)).
   - **Búsqueda cross-sucursal:** el `Customer` es una entidad global sin `branchId` ([schema.prisma:667](../..)) — la búsqueda **no filtra por sucursal**; cualquier cajera o admin ve clientes registrados por cualquier otra.
   - El DTO de query acepta `search: string` (mínimo 2 caracteres; si vacío o ausente, devuelve lista vacía o error de validación, a definir en [§6.2a](technical_guide.md#62a-customercreateresponse--customersearchresponse)).
   - Devuelve como máximo N resultados (ej. 20) ordenados por `updatedAt DESC` para priorizar clientes recientes.
...
1. `src/inventory/` (module/controller/service + dtos) — `POST /inventory/adjust` ([§6.8](technical_guide.md#68-inventoryadjustresponse-éxito), cablear `ADJUST_INVENTORY`), `PATCH /inventory/{id}/sale-price` (precio por presa — alimenta el sugerido custom, [PDR §2.10](../business/pdr.md#210-ventas-custom-presas-surtidas)), `GET /inventory/dashboard` (stock cocido + delta del turno, FR-006).
...
1. `src/customers/` — módulo CRUD completo sobre el `Customer` global (entidad sin `branchId`, cross-sucursal — [PDR §2.12](../business/pdr.md#212-clientes-y-facturación-nominada), [schema.prisma:667](../..)). Endpoints:
   - `GET /customers?search=&page=&pageSize=&status=&from=&to=` ([§6.2a](technical_guide.md#62a-customercreateresponse--customersearchresponse)) — búsqueda cross-sucursal con paginación y filtros (status, rango de fecha de registro). El `GET /customers?search=` mínimo ya quedó cableado en Sprint 1 paso 6; en Sprint 4 se **amplía** el query DTO con paginación, status, from, to (sin `registeredBranchId` — el `Customer` no tiene `branchId`; "sucursal de registro" se deriva del primer `Order` del cliente si se necesita, **fuera del alcance V1**).
   - `POST /customers` — alta; reglas [PDR §2.12](../business/pdr.md#212-clientes-y-facturación-nominada): `ci` UNIQUE (devolver `CUSTOMER_CI_ALREADY_EXISTS` si ya existe — la **primera registración gana**, no se permite duplicado cross-sucursal). `passwordHash` se mantiene `null` en V1 (reservado para V2 según [PDR §13.2](../business/pdr.md#132-versión-2-roadmap-futuro--diferido-explícitamente)).
   ...
   - `PATCH /customers/{id}/toggle-active` — **nuevo**, mismo patrón que `PATCH /users/{id}/toggle-active` y `PATCH /branches/{id}/toggle-active`: invierte el flag `active` actual, sin body. El `ADMINISTRADOR` puede desactivar clientes que no quiere que aparezcan en búsquedas futuras; las órdenes pasadas siguen mostrando el `customerName` snapshot. Clientes desactivados se pueden reactivar.
   - `GET /customers/{id}/orders?from=&to=` — **nuevo**, historial de pedidos del cliente (todas las sucursales, cross-sucursal). Reutiliza la query del módulo `orders` filtrada por `customerId` + rango de fechas opcional; el endpoint dedicado expone una vista resumida optimizada para el modal "Ver historial" de la UI administrativa.
   - **Roles:** `@Roles('CASHIER', 'ADMIN')` para `POST`, `GET /customers` y `GET /customers/{id}`. `@Roles('ADMIN')` para `PATCH /customers/{id}` y `PATCH /customers/{id}/toggle-active`. `@Roles('CASHIER', 'ADMIN')` para `GET /customers/{id}/orders` (la cajera puede ver el historial del cliente asociado a la orden actual). `SUPER_ADMIN` pasa siempre por la regla global.
...
| [FR-004](../business/requirements.md#fr-004--control-de-caja-por-turno-alta) | Control de caja por turno | 1 (open mínimo) + 3 (arqueo) |
| [FR-005](../business/requirements.md#fr-005--vales-de-trabajadores-media) | Vales | 3 |
| [FR-006](../business/requirements.md#fr-006--inventario-por-presas-alta) | Inventario por presas (2 planos) | 2 |
...
| [FR-019](../business/requirements.md#fr-019--registro-y-búsqueda-de-clientes-alta) | Clientes | 4 |
