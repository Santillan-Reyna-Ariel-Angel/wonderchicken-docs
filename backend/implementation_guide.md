...
6. `src/customers/` **(mínimo — solo lectura + lookup exacto)** — habilita el componente G del POS (selector de cliente en el resumen de la orden) para que la cajera tipee CI o NIT y asocie clientes pre-cargados a la orden. **Dos endpoints, ambos cross-sucursal, sin búsqueda con prefijo** para evitar llamadas innecesarias al backend mientras la cajera tipea. ✅ **IMPLEMENTADO**
   ...
   - `@Roles('CASHIER', 'ADMIN')` para ambos endpoints; `SUPER_ADMIN` pasa siempre por la regla global del `RolesGuard` ([§5.2](technical_guide.md#52-autenticación-y-autorización)).
   ...
   - **404 con código estable:** `error.code = 'CUSTOMER_NOT_FOUND'` para que el frontend pueda mostrar un mensaje claro ("No se encontró — usa S/N o esperá a Sprint 4 para crearlo").
   ...
   - Los endpoints de escritura (`POST`, `PATCH`, `PATCH /{id}/toggle-active`), `GET /customers/{id}` y `GET /customers/{id}/orders` llegan en Sprint 4 (paso 1).
