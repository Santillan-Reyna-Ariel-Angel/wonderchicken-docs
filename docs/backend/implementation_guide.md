...
1. `src/customers/` — `GET /customers?search=` (por CI/NIT/nombre), `POST`, `GET /{id}`, `PATCH /{id}` ([§6.2a](technical_guide.md#62a-customercreateresponse--customersearchresponse); reglas [PDR §2.1](../business/pdr.md#21-precios-y-sustituciones)2 — snapshot de `customerName` en la orden, editar cliente NO altera pedidos pasados).
