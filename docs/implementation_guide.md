...
2. `inventory.service.ts`: método transaccional `decrementForOrder(tx, items)` — regla de pares y bebidas por unidad (PDR §2.3), lanza `INSUFFICIENT_STOCK` ([§4.2](technical_guide.md#42-reglas-transaccionales)). La fuente de verdad son las filas `OrderItemComponent` de cada ítem (una por presa/bebida/extra consumido); el service itera esas filas para crear `InventoryTransaction` en la misma `$transaction` que confirma el pago. El `OrderItem.snapshot` (Json) se usa para impresión y auditoría, no para mover stock.
