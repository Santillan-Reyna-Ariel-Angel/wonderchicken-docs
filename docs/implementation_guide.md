...
> **Qué es este documento:** el **orden ejecutable** para construir el backend V1 — qué archivo crear en cada paso, sprint por sprint ([PDR §10](pdr.md#10-prioridad-de-trabajo-y-roadmap-de-entregas-mvp-en-sprints)), hasta cubrir todos los FR de V1 ([PDR §13](pdr.md#13-alcance-v1-mvp-vs-v2-futuro)). **Solo V1** — nada de V2.
>
> **Qué NO es:** no re-explica reglas ni contratos. Cada paso **referencia** dónde leerlos: la regla en [PDR §2](pdr.md#2-reglas-de-negocio-definitivas-y-no-negociables), el criterio en [FR de requirements](requirements.md#1-requerimientos-funcionales-completos-y-criterios-de-aceptación), el contrato en [technical guide §5](technical_guide.md#5-endpoints-contratos-y-seguridad-jwt), la capa en [architecture_overview §2](architecture_overview.md#2-flujo-interno-como-viaja-un-request-lifecycle). Ante conflicto: **gana el PDR** en negocio; ante conflicto de implementación, gana el código.
...
**Objetivo:** todo lo transversal que los demás sprints consumen ([PDR §10](pdr.md#10-prioridad-de-trabajo-y-roadmap-de-entregas-mvp-en-sprints), Sprint 0 = modelado, ya hecho; acá se agrega el arranque técnico).
**Cierra:** FR-018; base de FR-008 y FR-013 (backend como frontera de seguridad).
**Ajuste sobre [PDR §10](pdr.md#10-prioridad-de-trabajo-y-roadmap-de-entregas-mvp-en-sprints) (explicado):** el PDR no ubica la autenticación en ningún sprint, pero los guards son **globales** (`APP_GUARD`) y todos los endpoints los atraviesan → va primero o se retrofitea todo.
...
7. `src/users/` — `POST/GET/PATCH /users` (admin; alta con rol, activar/desactivar — [PDR §2.7](pdr.md#27-roles-e-interfaces-v1-con-autenticación-jwt-y-control-de-permisos-por-rol-en-backend)).
...
**Objetivo:** [PDR §10](pdr.md#10-prioridad-de-trabajo-y-roadmap-de-entregas-mvp-en-sprints), Sprint 1 — productos/variantes y venta MESA/LLEVAR con pago y comanda digital.
**Cierra:** [FR-001](requirements.md#fr-001--gestión-de-productos-y-variantes-alta), [FR-002](requirements.md#fr-002--registro-de-pedidos-pos-alta), [FR-003](requirements.md#fr-003--comanda-digital-y-factura-opcional-alta) (parcial: comanda digital), [FR-011](requirements.md#fr-011--pedidos-con-pago-pendiente-alta) (parcial: crear pendiente y pagar — sin inventario todavía, eso es Sprint 2 según [PDR §10](pdr.md#10-prioridad-de-trabajo-y-roadmap-de-entregas-mvp-en-sprints)).
**Ajustes sobre [PDR §10](pdr.md#10-prioridad-de-trabajo-y-roadmap-de-entregas-mvp-en-sprints) (explicados):**
- *Shifts mínimo se adelanta:* crear una orden exige turno abierto y `orderNumber` atómico vía `Shift.lastOrderNumber` ([PDR §2.8](pdr.md#28-comandas-tickets-factura-y-notificaciones)) — el arqueo completo queda en Sprint 3.
- *AuditService nace acá:* [PDR §10](pdr.md#10-prioridad-de-trabajo-y-roadmap-de-entregas-mvp-en-sprints) pone auditoría en Sprint 5, pero [§4.3](technical_guide.md#43-auditoría--implementación-v1) exige el `INSERT` **dentro de la misma transacción** de cada acción crítica — retrofitearlo en S5 significaría reabrir todos los services. Cada sprint cablea sus acciones; S5 solo completa y verifica.

Archivos, en orden:

1. `src/products/` (module/controller/service + `dto/create-product.dto.ts`, `create-variant.dto.ts`) — CRUD productos y variantes ([§6.1](technical_guide.md#61-productcreateresponse-éxito)/[§6.2](technical_guide.md#62-variantcreateresponse-éxito); componentes de variante: [PDR §2.2](pdr.md#22-variantes-y-componentes)).
2. `src/shifts/` **mínimo** — `GET /shift-periods` (catálogo; seed ya crea Mañana/Noche), `POST /shifts/open` ([§6.10](technical_guide.md#610-cashopenresponse-éxito); el período se **declara**, jamás se infiere del reloj — [PDR §13.3](pdr.md#133-decisiones-residuales-pendientes-de-cierre-antes-de-v1)) y el contador `lastOrderNumber`.
...
**Objetivo:** [PDR §10](pdr.md#10-prioridad-de-trabajo-y-roadmap-de-entregas-mvp-en-sprints) Sprint 2 — los dos planos del pollo (§2.3), decremento atómico al pagar, flujo custom, dashboard y consumos manuales.
**Cierra:** FR-002b, FR-006, FR-017; completa FR-011 (inventario recién al confirmar pago).

Archivos, en orden:

1. `src/inventory/` (module/controller/service + dtos) — `POST /inventory/adjust` ([§6.8](technical_guide.md#68-inventoryadjustresponse-éxito), cablear `ADJUST_INVENTORY`), `PATCH /inventory/{id}/sale-price` (precio por presa — alimenta el sugerido custom, [PDR §2.10](pdr.md#210-ventas-custom-presas-surtidas)), `GET /inventory/dashboard` (stock cocido + delta del turno, FR-006).
2. `inventory.service.ts`: método transaccional `decrementForOrder(tx, items)` — regla de pares y bebidas por unidad ([PDR §2.3](pdr.md#23-inventario-por-presas)), lanza `INSUFFICIENT_STOCK` ([§4.2](technical_guide.md#42-reglas-transaccionales)). La fuente de verdad son las filas `OrderItemComponent` de cada ítem (una por presa/bebida/extra consumido); el service itera esas filas para crear `InventoryTransaction` en la misma `$transaction` que confirma el pago. El `OrderItem.snapshot` (Json) se usa para impresión y auditoría, no para mover stock.
...
**Objetivo:** [PDR §10](pdr.md#10-prioridad-de-trabajo-y-roadmap-de-entregas-mvp-en-sprints) Sprint 3 — arqueo, gastos, vales, feature de descuentos con autorización por turno, reportes.
...
5. **Integrar en orders**: validación del `discountId` por ítem EN LA CREACIÓN (disponibilidad + autorización del turno → `DISCOUNT_NOT_AVAILABLE`/`DISCOUNT_NOT_AUTHORIZED`), snapshot por unidad y totales derivados ([§6.6](technical_guide.md#66-ordercreatewithdiscountresponse-descuento-al-personal-por-plato-en-la-creación--una-sola-llamada); regla completa [PDR §2.1](pdr.md#21-precios-y-sustituciones)1). Quitar el rechazo temporal del Sprint 1.
...
**Objetivo:** [PDR §10](pdr.md#10-prioridad-de-trabajo-y-roadmap-de-entregas-mvp-en-sprints) Sprint 4 — notificaciones de pedido listo, vista del cliente, factura.
**Cierra:** FR-007, FR-014, FR-015, FR-019; completa FR-003.

Archivos, en orden:

1. `src/customers/` — `GET /customers?search=` (por CI/NIT/nombre), `POST`, `GET /{id}`, `PATCH /{id}` ([§6.2a](technical_guide.md#62a-customercreateresponse--customersearchresponse); reglas [PDR §2.1](pdr.md#21-precios-y-sustituciones)2 — snapshot de `customerName` en la orden, editar cliente NO altera pedidos pasados).
2. `src/orders/` (completar) — `PATCH /orders/{id}/status` ([§6.11b](technical_guide.md#611b-orderstatusupdateresponse-despacho): `ready` persiste `readyAt`, `delivered` persiste `deliveredAt`+`deliveredBy` — [PDR §4](pdr.md#4-máquina-de-estados-de-pedidos)).
...
4. `src/print/` — `POST /print/invoice` (térmica, a demanda) y `GET /print/invoice/{orderId}/pdf` (fallback) — FR-014; formato del ticket: [PDR §7.7](pdr.md#77-factura-impresión--pdf) / business_context.
...
**Objetivo:** [PDR §10](pdr.md#10-prioridad-de-trabajo-y-roadmap-de-entregas-mvp-en-sprints) Sprint 5 — cerrar auditoría, sesión única, E2E y despliegue.
...
|---|---|---|
| [FR-001](requirements.md#fr-001--gestión-de-productos-y-variantes-alta) | Productos y variantes | 1 |
| [FR-002](requirements.md#fr-002--registro-de-pedidos-pos-alta) / [FR-002b](requirements.md#fr-002b--venta-custom-de-presas-surtidas-alta) | POS estándar / venta custom | 1 / 2 |
| [FR-003](requirements.md#fr-003--comanda-digital-y-factura-opcional-alta) | Comanda digital y factura opcional | 1 (comanda) + 4 (factura) |
| [FR-004](requirements.md#fr-004--control-de-caja-por-turno-alta) | Control de caja por turno | 1 (open mínimo) + 3 (arqueo) |
| [FR-005](requirements.md#fr-005--vales-de-trabajadores-media) | Vales | 3 |
| [FR-006](requirements.md#fr-006--inventario-por-presas-alta) | Inventario por presas (2 planos) | 2 |
| [FR-007](requirements.md#fr-007--notificación-de-pedido-listo-alta) | Pantalla pública de pedidos listos | 4 |
| [FR-008](requirements.md#fr-008--interfaces-diferenciadas-por-rol-alta) / [FR-008b](requirements.md#fr-008b--sesión-única-por-turno-alta) | Permisos por rol / sesión única | 0 (guards) + 5 (sesión) |
| [FR-009](requirements.md#fr-009--registro-de-gastos-media) | Gastos desde caja | 3 |
| [FR-010](requirements.md#fr-010--reportes-alta) | Reportes + CSV | 3 |
| [FR-011](requirements.md#fr-011--pedidos-con-pago-pendiente-alta) / [FR-011b](requirements.md#fr-011b--anulación-de-pedido-pagado-alta) | Pago pendiente / anulación | 1-2 / 2 |
| [FR-012](requirements.md#fr-012--historial-de-comandas-media) | Historial de comandas | 1 (`GET /orders`) |
| [FR-013](requirements.md#fr-013--ui-limpia-por-rol-alta) | UI limpia por rol | transversal (la matriz §5.2 es la spec; UI = frontend) |
| [FR-014](requirements.md#fr-014--impresión-de-factura-y-descarga-pdf-media) | Factura térmica + PDF | 4 |
| [FR-015](requirements.md#fr-015--vista-pública-del-cliente-alta) | Vista pública del cliente | 4 |
| [FR-016](requirements.md#fr-016--gestión-de-descuentos-y-aplicación-en-pos-media) / [FR-016b](requirements.md#fr-016b--autorización-de-descuentos-por-turno-media) | Descuentos / autorización por turno | 3 |
| [FR-017](requirements.md#fr-017--registro-de-consumos-manuales-y-ciclo-crudo-de-presas-por-turno-media) | Consumos manuales + ciclo crudo | 2 |
| [FR-018](requirements.md#fr-018--autenticación-jwt-y-autorización-por-rol-alta) | JWT + autorización | 0 |
| [FR-019](requirements.md#fr-019--registro-y-búsqueda-de-clientes-alta) | Clientes | 4 |
