...
### Cliente como entidad global (cross-sucursal)

- **Alcance del cliente:** el `Customer` es una **entidad global**, sin `branchId`. Un cliente registrado en cualquier sucursal es **visible y reutilizable por todas** las demás. Esto evita duplicados por CI/NIT cuando una misma persona compra en más de una sucursal y mantiene la identidad del cliente coherente para FR-015 ("pedidos del día").
- **Quién puede registrar y consultar:**
  - `CAJERA` registra clientes en el flujo operativo (modal de alta rápida desde el POS, atajo F9) y los asocia a la orden en curso.
  - `ADMINISTRADOR` registra, consulta (incluye vista cross-sucursal), edita, ve el historial de pedidos del cliente y activa/desactiva (`toggle-active`). Lo hace desde una vista administrativa dedicada.
  - `SUPER_ADMIN` tiene visibilidad global cross-sucursal y puede operar sobre cualquier cliente.
- **Búsqueda cross-sucursal:** la búsqueda por CI o NIT **no filtra por sucursal**. Si una cajera de la Sucursal A busca un CI ya registrado en la Sucursal B, lo encuentra y lo reutiliza.
- **Deduplicación:** si al intentar registrar un cliente el `CI` ya existe, el sistema lo muestra y **NO permite crear un duplicado**. La primera registración gana. Esto aplica también al NIT cuando viene informado.
- **Edición y snapshots:** editar los datos de un cliente **no propaga** a los `customerName` ya guardados como snapshot en órdenes pasadas. El cliente nuevo se ve en operaciones futuras; el pasado permanece inmutable.
- **Activación / desactivación:** el cliente lleva `active: boolean` (default `true`). El `ADMINISTRADOR` puede desactivarlo; las órdenes pasadas siguen mostrando el snapshot del nombre. Los clientes desactivados **no aparecen** en búsquedas futuras, pero pueden reactivarse.
- **Vinculación a la orden (sin cambios):** `Order.customerId` es opcional. Si no se identifica, la orden queda como "S/N" (regla legal de nominatividad, ventas < Bs 1.000).
- **Pedidos del día (FR-015) cross-sucursal:** desde la vista del cliente (acceso por `publicToken`) se listan **todos** los pedidos del cliente emitidos durante el día, **en cualquier sucursal**, agrupados por cliente. La venta anónima "S/N" solo ve su pedido único.
...
| Comandas, tickets, factura y vista pública | Comandas digitales por default; historial con búsqueda; vista pública por token + pedidos del día para clientes registrados; clientes como entidad **global cross-sucursal** para factura nominada (anónimo = S/N), gestionados por `CAJERA` (alta rápida operativa) y `ADMINISTRADOR` (gestión: consulta cross-sucursal, edición, historial, activar/desactivar); factura solo a demanda (térmica con fallback PDF); tipos de ticket solo MESA y LLEVAR. | [FR-003](requirements.md#fr-003--comanda-digital-y-factura-opcional-alta), [FR-012](requirements.md#fr-012--historial-de-comandas-media), [FR-014](requirements.md#fr-014--impresión-de-factura-y-descarga-pdf-media), [FR-015](requirements.md#fr-015--vista-pública-del-cliente-alta), [FR-019](requirements.md#fr-019--registro-y-búsqueda-de-clientes-alta); [§2.7](#27-roles-e-interfaces-v1-con-autenticación-jwt-y-control-de-permisos-por-rol-en-backend), [§2.8](#28-comandas-tickets-factura-y-notificaciones), [§2.10](#210-ventas-custom-presas-surtidas), [§2.12](#212-clientes-y-facturación-nominada), [§7.4](#74-vista-del-cliente-su-comanda) |
