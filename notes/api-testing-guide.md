...
| 17 | `POST` | `/orders` | `cashierToken` | `{ "type": "MESA", "tableNumber": 5, "items": [{ "productId": "{{productId}}", "quantity": 2 }] }` | `201` → guardar `orderId`, `publicToken` |
...
| 20 | `GET` | `/orders` | `cashierToken` | `?status=CONFIRMED` | `200` → órdenes pagadas |
...
| # | Método | Endpoint | Auth | Body | Esperado |
|---|--------|----------|------|------|---------|
| 21 | `POST` | `/orders` | `cashierToken` | `{ "type": "LLEVAR", "items": [{ "productId": "{{productId}}", "quantity": 1 }] }` | `201` → guardar `orderId2` |
...
| 25 | `GET` | `/orders/:orderId` | `dispatcherToken` | — | `200` → DISPATCHER ve órdenes de su sucursal |
...
| 36 | `GET` | `/users` | *(sin token)* | — | `401` → `Token de autenticación requerido` |
